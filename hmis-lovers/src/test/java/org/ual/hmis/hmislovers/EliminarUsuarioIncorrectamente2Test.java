...
public class EliminarUsuarioIncorrectamente2Test {
  ...
  @Test
  public void loginCasoincorrectoCredencialesinvlidas() {
    ...
    // 3 | click | Esperar y seleccionar el campo de usuario
    WebElement inputUsername = wait.until(ExpectedConditions.visibilityOfElementLocated(By.id("login-username")));
    inputUsername.click();

    // 4 | type | Introducir el usuario inválido
    inputUsername.sendKeys("aiaia");

    // 5 | click | Seleccionar contraseña
    WebElement inputPassword = wait.until(ExpectedConditions.visibilityOfElementLocated(By.id("login-password")));
    inputPassword.click();

    // 6 | type | Introducir clave incorrecta
    inputPassword.sendKeys("eeee");

    // 7 | click | Esperar a que el botón sea plenamente clickeable y pulsarlo
    ...
    assertTrue(botonSigueVisible.isDisplayed());
  }
}